...
class TestSerialization(unittest.TestCase):
    ...
    def create_accounting(self):
        ...
        """
        acc = Accounting()
        acc._petrol_price.add("Ai92", 22)
        acc.add("test", "test", 200)
        acc._charge_of_gasoline.add_car("zaz01", 10)
    ...
    def test_yaml(self):
        ...
        for p1, p2 in zip(obj._expense, obj1.expense):
            self.assertEqual(p1, p2)
        for p1, p2 in zip(obj._petrol_price.price, obj1.petrol_price.price):
            self.assertEqual(p1, p2)
        for p1, p2 in zip(obj._charge_of_gasoline.gasoline, obj1.charge_of_gasoline.gasoline):
            self.assertEqual(p1, p2)
