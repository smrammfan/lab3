    ...
    def car_menu(self):
        ...
        while True:
            ...
            if option == '1':
                ...
                self.accounting._charge_of_gasoline.add_car(c_name, c_charge)
            elif option == '2':
                c_name = self.interface.input_my("Enter car name to delete.")
                self.accounting._charge_of_gasoline.del_car(c_name)
            elif option == '3':
                c_name = self.interface.input_my("Enter car name to change.")
                c_charge = float(self.interface.input_my("Enter new car charge."))
                self.accounting._charge_of_gasoline.change_car(c_name, c_charge)
            elif option == '4':
                c_name = self.interface.input_my("Enter car name to check charge.")
                self.accounting._charge_of_gasoline.get_charge_of_car(c_name)
            elif option == '5':
